...
class Node: NSObject {
	...
	init(name: String, vertices: Array<Vertex>, device: MTLDevice){
		...
		uniformsBuffer = device.newBufferWithLength(sizeof(Float)*16*2, options: MTLResourceOptions.OptionCPUCacheModeDefault)
		super.init()
	}
	...
	func sceneAdjustedUniformsBufferForworldModelMatrix(worldModelMatrix: Matrix4, projectionMatrix: Matrix4) -> MTLBuffer {
		...
		memcpy(bufferPointer!, nodeModelMatrix.raw(), Int(sizeof(Float)*16))
		memcpy(bufferPointer! + sizeof(Float)*16, projectionMatrix.raw(), Int(sizeof(Float)*16))
